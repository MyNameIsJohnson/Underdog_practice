...
function getAllWordsWithXYZ(str) {
  var results = [];
  for (var i = 0; i < str.length; i++) {
    if (str[i].includes("X") && str[i].includes("Y") && str[i].includes("Z")) {
      results.push(str[i]);
    }
  }
  return results;
...
const getWordsWithGHTLY = (words) => {
  ...
  for (var i = 0; i < words.length; i++) {
    let str = "GHTLY";
    if (words[i].includes(str)) wordsWithGHTLY.push(words[i]);
  }
...
const getLongestWordWithoutVowels = (words) => {
  ...
  var longestWordWithoutVowels = getWordsWithoutVowels.sort(
...
function isPalindrome(str) {
  ...
  if (str.length > 0 && str === strReverse) {
    return true;
  } else {
    return false;
  }
